...
def sub_kb():
   ...
   kb.add(types.InlineKeyboardButton("📢 Kanalga a'zo bo'lish", url=f"https://t.me/DLS_UZB_UZB"))
   kb.add(types.InlineKeyboardButton("✅ A'zo bo'ldim", callback_data="check_sub"))
   return kb
...
def season_pass(message):
   if not is_subscribed(message.from_user.id):
       bot.send_message(message.chat.id, "⚠️ Kanalga a'zo bo'ling!", reply_markup=sub_kb())
       return
