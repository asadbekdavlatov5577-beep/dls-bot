...
def contact_kb():
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📩 @DAVLATOV_5577 bilan bog'lanish", url="https://t.me/DAVLATOV_5577")]
    ])
    return kb
...
async def account_detail(callback: types.CallbackQuery):
    ...
    text = f"👤 *Account #{acc_id[-1]}*\n\n📞 Narx uchun adminga murojaat qiling!"
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📩 Admin", url="https://t.me/DAVLATOV_5577")]
    ])
    if img:
...
async def dls_haqida(message: types.Message):
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━━━\n\n"
        "🌟 Yangi grafika va animatsiyalar\n"
        "⚽ 5000+ real futbolchi\n"
        "🏟 Yangi stadionlar\n"
        "🌐 Onlayn multiplayer\n"
        "🎫 Season Pass tizimi\n\n"
...
async def reyting(message: types.Message):
    text = (
        ...
        "🥈 2. @user2 — 4 xarid\n"
        "🥉 3. @user3 — 3 xarid\n\n"
        "Ko'proq xarid qiling va TOP'ga kiring! 🚀"
    )
    await message.answer(text, parse_mode="Markdown", reply_markup=main_menu())
...
async def choose_image_key(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"✅ Yangi rasmni yuboring:")
    await callback.answer()
