...
def main_menu():
   ...
   kb.add(types.KeyboardButton("👤 Accountlar"), types.KeyboardButton("ℹ️ DLS26 Haqida"))
   kb.add(types.KeyboardButton("📞 Murojaat"), types.KeyboardButton("🏆 Reyting"))
   return kb
...
def admin_broadcast(call):
   if call.from_user.id != ADMIN_ID:
       bot.answer_callback_query(call.id, "❌ Ruxsat yo'q!")
       return
   bot.send_message(call.message.chat.id, "📢 E'lon yuborish tez orada!")
   bot.answer_callback_query(call.id)
